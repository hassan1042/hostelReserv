...
const Navbar = () => {
  ...
  return (
    <nav className={`bg-bgUI dark:bg-bgUIDark px-4 max-md:py-2 text-black dark:text-black  ${headerSticky ? 'fixed top-0 ' : ''} z-50 w-full`}>
      <div className="container mx-auto flex justify-between  max-lg:flex-col lg:items-center">
        <NavLink to="/" className="text-white text-lg font-bold w-12 h-12 lg:h-16 lg:w-16 max-[220px]:me-3">
  ...
  <DarkMode/>
          ...
          <div className={`absolute right-5 top-5 max-[220px]:text-xl max-[220px]:right-2 text-2xl text-white lg:hidden hover:text-hovLinks`} onClick={handleMenu}>
      <FaBars />
      </div>
        </div>
      ...
      <div className={`absolute right-32 top-5 text-2xl text-white lg:hidden max-[220px]:text-xl max-[220px]:right-24`} >
      <Notifications />
      </div>
      <div className={`absolute right-20 top-4 text-xl text-white lg:hidden max-[220px]:text-xl max-[220px]:right-12`} >
      <DarkMode />
      </div>
    </nav>
  );
